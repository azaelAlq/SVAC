...
def iniciar_interfaz():
    ...
    entry_duracion.insert(0, "1000")  # Por defecto 1000 ms
    ...
    tk.Button(control_frame, text="Velocidad Lenta", command=lambda: enviar_comando_motores("motor/velocidad/lenta")).grid(row=5, column=0)
    tk.Button(control_frame, text="Velocidad Normal", command=lambda: enviar_comando_motores("motor/velocidad/normal")).grid(row=5, column=2)
        ...
        if cono_mas_cercano:
            ...
            if modo_autonomo_activado:
                if 14 <= distancia_cm <= 17:
                    ...
                    mover("izquierda", 100)  # Mover con duración de 100 ms
                elif centro_cono > 390:
                    mover("derecha", 100)  # Mover con duración de 100 ms
                else:
                    mover("adelante", 100)  # Mover con duración de 100 ms
        else:
            if modo_autonomo_activado:
                mover("giro", 100)  # Mover con duración de 100 ms
